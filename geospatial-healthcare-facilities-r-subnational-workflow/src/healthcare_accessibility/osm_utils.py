...
def acquire_latest_osm_data(country, continent, data_dir):
    """
    ...
    data_dir : str or pathlib.Path
        The path to the root data directory for the project.
    """

    country = country.lower()
    continent = continent.lower()

    output_filepath = data_dir + f"{country}-latest.osm.pbf"

    generate_output_bool = continue_confirmation(output_filepath)

    if generate_output_bool:
...
def return_points_of_interest(osm, amenity_types, plot_positions=False):
    """
    Return points of interest of a specific amenity type from the OSM object.

    For full amenities list see:
      https://wiki.openstreetmap.org/wiki/Map_features#Healthcare

    Parameters:
    - osm_obj: OSM object
    - amenity_types: list, type of amenity to filter by
    - plot_positions: bool, whether to visualise positions of the points of interest

    Returns:
    - GeoDataFrame containing points of interest of the specified amenity type
    """
    poi_df = osm.get_pois({"amenity": True})
    mask = poi_df["amenity"].isin(amenity_types)
    focus_df = poi_df[mask]
...
def load_driving_network(osm, plot_road_network=False):
    """
    Use the pyrosm package to load the driving road network from an OSM object.

    Parameters
    ----------
    osm : object
        OSM object return by pyrosm
    plot_road_network : bool, optional
        If True, generate visual of road network, by default False
...
def clean_queried_osm_roads(gdf, keep_metadata):
    """
    Cleans and standardizes a GeoDataFrame containing road data queried from OSM.

    Parameters:
    ----------
    gdf : geopandas.GeoDataFrame
        The input GeoDataFrame containing road network data.

    keep_metadata : str
        Flag indicating whether to retain metadata columns.
        - "YES": Retains most metadata except 'osmid'.
        - "NO": Drops a predefined set of metadata columns likely to contain missing values.

    Returns:
    -------
    geopandas.GeoDataFrame
        A cleaned GeoDataFrame with:
        - Selected metadata columns dropped based on the flag.
        - 'length' column rounded to 1 decimal place.
        - Missing values filled with "Unclassified".
        - String column values stripped of whitespace and capitalized (title case).
    """

    if keep_metadata == "YES":

        # Drop osmid
        gdf.drop(columns=["osmid"], inplace=True)

    elif keep_metadata == "NO":

        # Drop these columns as most likely missing lots of values
        gdf.drop(
            columns=[
                "osmid",
                "bridge",
    ...
    # Change length column to 1dp
    gdf["length"] = gdf["length"].round(1)

    # Fill null values
    gdf = gdf.fillna("Unclassified")

    # Loop through only str columns to remove white space and cap first letter of each word
