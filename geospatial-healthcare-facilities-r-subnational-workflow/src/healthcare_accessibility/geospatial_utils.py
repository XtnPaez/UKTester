...
def plot_interactive_travel_times_map(
    map, travels_gdf_vis, destination_hcf, fac_type, visualisation_crs
):
    """
    ...
    group = folium.FeatureGroup(name=fac_type).add_to(map)

    folium.Choropleth(
        geo_data=travels_gdf_vis.to_json(),
        data=travels_gdf_vis,
        name=f"{fac_type} travel time",
        columns=["id", "travel_time"],
        key_on="feature.properties.id",
        legend_name=f"Minutes to nearest {fac_type} facility",
        fill_opacity=0.7,
        line_weight=0.2,
        fill_color="viridis",
    ).add_to(map)
    ...
    for _, row in destination_hcf.to_crs(visualisation_crs).iterrows():
        # Metadata
        tooltip = (
            f"Facility Name: {row['Facility Name']}<br>"
            f"Facility Type: {row['Facility Type']}<br>"
            f"Ownership: {row['Facility Ownership']}<br>"
            f"Status: {row['Status']}<br>"
            # f"Area Classification: {row['Facility Location']}<br>"
            # f"Information: {row['Facility Information']}<br>"
...
def plot_travel_times_map(
    travel_gdf, districts_gdf, destination_point, analysis_crs, savefig=False
):
    """
    ...
    districts_gdf : GeoDataFrame
        GeoDataFrame of administrative boundaries (e.g., districts) to overlay on the map.
...
def evaluate_travel_times_to_facilities(
    ...
    """
    Calculates travel times from population grid centroids to a healthcare facility using a specified transport mode.

    Parameters
    ----------
    origin_grids : GeoDataFrame
        GeoDataFrame containing population grid cells to be used as origin points.

    destination_point : GeoDataFrame
        GeoDataFrame containing the destination healthcare facility (or facilities).
    ...
    districts_gdf : GeoDataFrame
        GeoDataFrame of district boundaries, used for optional map visualization.
    ...
    unit_of_time : str
        Unit of time for `travel_time`. Must be either "minutes" or "hours".

    output_map : bool, optional (default=True)
        If True, displays a map showing travel times from origin grids to the destination.
    ...
    if unit_of_time == "minutes":
        ...
        seconds = travel_time * 3600
    else:
        raise ValueError("unit_of_time must be either 'minutes' or 'hours'")

    # Reproject all GeoDataFrames to the analysis CRS for consistent spatial operations
    origin_grids = origin_grids.to_crs(analysis_crs)
    destination_point = destination_point.to_crs(analysis_crs)
    districts_gdf = districts_gdf.to_crs(analysis_crs)
    ...
    # Optionally plot the travel time map
    if output_map:
        geo_util.plot_travel_times_map(
            travel_gdf, districts_gdf, destination_point, analysis_crs=analysis_crs
        )
...
def calc_cumulative_percentage(gdf, sub_total_pop):
    """
    ...
    # convertin to cumulative %
    total_population = sub_total_pop["population"].sum()
