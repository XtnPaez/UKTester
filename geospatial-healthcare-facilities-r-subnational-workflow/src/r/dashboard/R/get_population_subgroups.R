...
#' @param filepath Either: (a) path to a pre-extracted folder of WorldPop .tif
#'   files (returned as-is), or (b) path to a WorldPop ZIP file to extract, or
#'   NULL/missing to trigger automatic download via \code{country}.
#' @param year WorldPop data year.
#' @param dest_dir The directory to save the downloaded ZIP file (default: "data/raw").
#' @param zip_name The name to save the downloaded ZIP file as (default: derived
#'   from ISO3 code and year).
#' @param unzip_folder The directory to extract the ZIP file to (default: same as
#'   dest_dir).
#' 
#' @return The path to the folder containing the extracted .tif files.
get_subgroup_population_files <- function(
  ...
  if (missing(filepath) || is.null(filepath) || identical(filepath, "")) {
    ...
    message("Using pre-extracted WorldPop folder: ", filepath)
    return(invisible(filepath))
  }

  if (is.null(iso3)) {
    if (is.null(country) || !nzchar(country)) {
      stop("country must be set to derive the extraction folder name when population_filepath is a ZIP.")
    }
    iso3 <- countrycode::countrycode(country, "country.name", "iso3c")
  }
...
#' Clip a population SpatRaster to district boundaries and write to disk
#'
#' Crops and masks the raster to the union of all provided district polygons,
#' retaining any cell that overlaps the boundary (touches = TRUE). The clipped
#' raster is written to data/population/<area_name_lower>_agesex_<year>/geodemographics.tif.
#'
#' @param population_raster A SpatRaster (e.g., the geodemographics stack).
#' @param districts An sf object of district polygons defining the area.
#' @param area_name Character. Name used for the output subfolder (e.g., "Buenos Aires").
#' @param year WorldPop year, used in the output folder name.
#' @param out_dir Base directory for population output (default: data/population).
#' @param touches Logical. If TRUE (default), keep cells that merely touch the boundary.
#' @return Invisibly returns the path to the written raster file.
#' @importFrom terra crop mask vect writeRaster
#' @importFrom sf st_union st_as_sf
#' @export
clip_population_to_districts <- function(
    population_raster,
    districts,
    area_name,
    year,
  ...
  area_folder <- paste0(tolower(area_name), "_agesex_", year)
  out_path    <- file.path(out_dir, area_folder, "geodemographics.tif")
  dir.create(dirname(out_path), showWarnings = FALSE, recursive = TRUE)

  message("Clipping population raster to subnational boundary...")
  boundary_vect <- terra::vect(sf::st_union(districts))
  clipped <- terra::crop(population_raster, boundary_vect) |>
    terra::mask(boundary_vect, touches = touches)

  terra::writeRaster(clipped, out_path, overwrite = TRUE)
  message("Clipped population raster written to: ", out_path)
  invisible(out_path)
}
