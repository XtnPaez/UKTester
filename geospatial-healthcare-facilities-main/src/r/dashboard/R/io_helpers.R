...
#' @export
load_ttm <- function(country) {
  path <- here::here("data", "ttm", paste0(tolower(country), "_closest_times.parquet"))
...
#' Derives the ISO3 country code automatically via countrycode.
#'
#' @param country Country name (e.g., "Rwanda")
#' @param year Integer WorldPop year (e.g., 2025)
#' @return A SpatRaster from terra
#' @importFrom terra rast
#' @export
load_population_raster <- function(country, year) {
  iso3   <- tolower(countrycode::countrycode(country, "country.name", "iso3c"))
